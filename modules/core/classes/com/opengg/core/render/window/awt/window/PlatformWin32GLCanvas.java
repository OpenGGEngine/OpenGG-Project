...
import static com.opengg.core.render.window.awt.window.GLUtil.*;
...
public class PlatformWin32GLCanvas implements PlatformGLCanvas {
    ...
    public long recreate(Canvas canvas, GLData attribs, GLData effective, long oldCtx) throws AWTException {
        this.ds = JAWT_GetDrawingSurface(canvas, awt.GetDrawingSurface());
        JAWTDrawingSurface ds = JAWT_GetDrawingSurface(canvas, awt.GetDrawingSurface());
        try {
            int lock = JAWT_DrawingSurface_Lock(ds, ds.Lock());
            ...
            try {
                ...
                try {
                    ...
                    long hwndDummy = createDummyWindow();
                    try {
                        return recreate(hwnd, hwndDummy, attribs, effective, oldCtx);
                    } finally {
                        User32.DestroyWindow(hwndDummy);
                    }
                } finally {
    ...
    private long recreate(long windowHandle, long dummyWindowHandle, GLData attribs, GLData effective, long oldCtx) throws AWTException {
        MemoryStack stack = MemoryStack.stackGet();
        long bufferAddr = stack.nmalloc(4, (4*2) << 2);
    ...
    public void setNoContext(long context){
        WGL.wglMakeCurrent(0, context);
    }
