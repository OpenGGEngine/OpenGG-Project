    ...
    public static Model autoSimplifyToTargetIndexCount(Model model, int targetIndexCount) {
        ...
        for (var mesh : model.getMeshes()) {
            ...
            if (mesh.isTriStrip()) {
                ...
                var newIndices = Allocator.allocInt((int) size);
            ...
            var vertices = Allocator.allocFloat(mesh.getVertices().size() * 3);
            for(var vertex : mesh.getVertices()) {
                vertices.put(vertex.position.x).put(vertex.position.y).put(vertex.position.z);
            }
            vertices.rewind();

            var newIndices = Allocator.allocInt(targetIndexCount);
           // var optimizedListSize = MeshOptimizer.meshopt_simplifySloppy(newIndices, indices, vertices, mesh.getVertices().size(), 12, targetIndexCount);
            ...
            if (mesh.isTriStrip()) {
                ...
                var simpifiedStripIndices = Allocator.allocInt((int) size);
