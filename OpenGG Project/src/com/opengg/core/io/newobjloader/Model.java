 ...
 * @author Warren
 */
public class Model implements ModelMaterialDefinitions{
...
//        log.log(INFO,"Added texture  vertex " + verticesT.size() + " = " + verticesT.get(verticesT.size() - 1));
    ...
    public void addPoints(int[] values) {
        log.log(INFO, "Got {0} points in builder, ignoring", values.length);
    }

    @Override
    public void addLine(int[] values) {
        log.log(INFO, "Got a line of {0} segments in builder, ignoring", values.length);
    }
...
//            log.log(INFO,"Adding vertex g=" + vertexIndices[loopi] + " t=" + vertexIndices[loopi + 1] + " n=" + vertexIndices[loopi + 2]);
            ...
            if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesG.size())) {
                ...
                log.log(SEVERE, "Index for geometric vertex={0} is out of the current range of geometric vertex values 1 to {1}, ignoring", new Object[]{vertexIndex, verticesG.size()});
            }
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesT.size())) {
                    ...
                    log.log(SEVERE, "Index for texture vertex={0} is out of the current range of texture vertex values 1 to {1}, ignoring", new Object[]{vertexIndex, verticesT.size()});
                }
            }
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesN.size())) {
                    ...
                    log.log(SEVERE, "Index for vertex normal={0} is out of the current range of vertex normal values 1 to {1}, ignoring", new Object[]{vertexIndex, verticesN.size()});
                }
            }
...
//        log.log(INFO,"Parsed face=" + face);
    ...
    public void addMapLib(String[] names) {
        if (null == names) {
            log.log(INFO, "ERROR! Got a maplib line with null names array");
            return;
        }
        if (names.length == 1) {
            log.log(INFO, "Got a maplib line with one name=|{0}|", names[0]);
            return;
        }
        log.log(INFO, "Got a maplib line;");
