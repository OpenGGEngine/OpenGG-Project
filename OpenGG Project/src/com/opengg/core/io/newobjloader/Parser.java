...
public class Parser {
    ...
    /*public Parser(Model builder, String filename) throws FileNotFoundException, IOException {
        this.model = builder;
        model.setObjFilename(filename);
        parseObjFile(filename);

        model.doneParsingObj(filename);
    }*/
    public Parser(){
        
    }
    public Model parseModel(String filename) throws IOException{
    ...
    private void processVertex(String line) {
        ...
        model.addVertexGeometric(values[0], values[1], values[2]);
    }
    ...
    private void processVertexTexture(String line) {
        ...
        model.addVertexTexture(values[0], values[1]);
    }
    ...
    private void processVertexNormal(String line) {
        ...
        model.addVertexNormal(values[0], values[1], values[2]);
    }
...
//        System.err.println("Parsed as "+parsedList);
    ...
    private void processGroupName(String line) {
        ...
        model.setCurrentGroupNames(groupnames);
    }
    ...
    private void processObjectName(String line) {
        model.addObjectName(line.substring(OBJ_OBJECT_NAME.length()).trim());
    }
    ...
    private void processSmoothingGroup(String line) {
       ...
       model.setCurrentSmoothingGroup(groupNumber);
    }
        ...
        if (null != matlibnames) {
            for (String matlibname : matlibnames) {
                try {
                    ...
                    log.log(SEVERE, "Can''t find material file name=''{0}'', e={1}", new Object[]{matlibname, e});
                }
            }
        }
    }
    ...
    private void processUseMaterial(String line) {
        model.setCurrentUseMaterial(line.substring(OBJ_USEMTL.length()).trim());
    }
    ...
    private void processMapLib(String line) {
        ...
        model.addMapLib(maplibnames);
    }

    private void processUseMap(String line) {
        model.setCurrentUseMap(line.substring(OBJ_USEMAP.length()).trim());
    }
    ...
    private void parseMtlFile(String mtlFilename) throws FileNotFoundException, IOException {
        ...
        File mtlFile = new File(objFile.getParent().concat("/mtls/"), mtlFilename);
    ...
    private void processReflectivityTransmissivity(String fieldName, String line) {
        int type = ModelMaterialDefinitions.MTL_KA;
        switch (fieldName) {
            case MTL_KD:
                type = ModelMaterialDefinitions.MTL_KD;
                break;
            case MTL_KS:
                type = ModelMaterialDefinitions.MTL_KS;
                break;
            case MTL_TF:
                type = ModelMaterialDefinitions.MTL_TF;
                break;
        }

        String[] tokens = StringUtils.parseWhitespaceList(line.substring(fieldName.length()));
        if (null == tokens) {
            log.log(SEVERE, "Got Ka line with no tokens, line = |{0}|", line);
            return;
        }
        if (tokens.length <= 0) {
            log.log(SEVERE, "Got Ka line with no tokens, line = |{0}|", line);
...
// 	    }
            case "xyz":
                ...
                if (tokens.length < 2) {
                    log.log(SEVERE, "Got xyz line with not enough x/y/z tokens, need at least one value for x, found {0} line = |{1}|", new Object[]{tokens.length - 1, line});
                    return;
    ...
    private void processIllum(String line) {
        ...
        if ((illumModel < 0) || (illumModel > 10)) {
            log.log(SEVERE, "Got illum model value out of range (0 to 10 inclusive is allowed), value={0}, line={1}", new Object[]{illumModel, line});
            return;
        }
        model.setIllum(illumModel);
    }
    ...
    private void processMapDecalDispBump(String fieldname, String line) {
        int type = ModelMaterialDefinitions.MTL_MAP_KA;
        switch (fieldname) {
            case MTL_MAP_KD:
                type = ModelMaterialDefinitions.MTL_MAP_KD;
                break;
            case MTL_MAP_KS:
                type = ModelMaterialDefinitions.MTL_MAP_KS;
                break;
            case MTL_MAP_NS:
                type = ModelMaterialDefinitions.MTL_MAP_NS;
                break;
            case MTL_MAP_D:
                type = ModelMaterialDefinitions.MTL_MAP_D;
                break;
            case MTL_DISP:
                type = ModelMaterialDefinitions.MTL_DISP;
                break;
            case MTL_DECAL:
                type = ModelMaterialDefinitions.MTL_DECAL;
                break;
            case MTL_BUMP:
                type = ModelMaterialDefinitions.MTL_BUMP;
                break;
        }

        String filename = line.substring(fieldname.length()).trim();
        model.setMapDecalDispBump(type, filename);
